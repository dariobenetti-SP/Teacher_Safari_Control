...
Script INDIPENDENTE, pensato per essere eseguito una volta al giorno (es. alle 13:40
...
def e_da_saltare_oggi():
    ...
    try:
        ...
        # Se il foglio Festivita non esiste ancora o c'è un errore temporaneo, meglio
        # procedere comunque piuttosto che saltare erroneamente un giorno di scuola.
...
def main():
    ...
    for classe, ids in MAPPA_CLASSI.items():
        print(f"Rilascio serale: sblocco {classe}...")
        if sposta_dispositivi(ids["bloccata"], ids["libera"]):
            scrivi_log(sheet, "SBLOCCO_SERALE", classe)
            print(f"  ✓ {classe} sbloccata per il pomeriggio.")
        else:
            print(f"  ✗ ERRORE nello sblocco serale di {classe}.")
